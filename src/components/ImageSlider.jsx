...
function ImageSlider({ pictures }) {
  ...
  return (
    <div>
      <div className="relative overflow-x-hidden flex aspect-video h-full w-full">
        {slider}
        <button
          role="button"
          className="group absolute left-0 top-0 h-full w-[18%] rounded-l-lg duration-300 hover:bg-semiDark"
          onClick={previousSlide}
        >
          <span className="sliderBtn left float-left ml-[40%] border-b-8 border-r-8 p-2 duration-300 group-hover:ml-[30%] xs:p-3"></span>
        </button>
        <button
          role="button"
          className="group sliderBtn2 absolute right-0 top-0 h-full w-[12%] rounded-r-lg duration-300 hover:bg-semiDark"
          onClick={nextSlide}
        >
